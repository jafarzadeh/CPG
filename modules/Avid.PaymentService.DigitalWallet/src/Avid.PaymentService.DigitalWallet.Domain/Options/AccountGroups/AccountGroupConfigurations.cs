...
public class AccountGroupConfigurations
{
    ...
    public AccountGroupConfigurations Configure<TAccountGroup>(Action<AccountGroupConfiguration> configureAction)
    {
        return Configure(AccountGroupNameAttribute.GetAccountGroupName<TAccountGroup>(), configureAction);
    }

    public AccountGroupConfigurations Configure([NotNull] string name,
        [NotNull] Action<AccountGroupConfiguration> configureAction)
    {
        Check.NotNullOrWhiteSpace(name, nameof(name));
        Check.NotNull(configureAction, nameof(configureAction));
        configureAction(_accountGroups.GetOrAdd(name, () => new AccountGroupConfiguration()));
        return this;
    }
    ...
    {
        return _accountGroups.Where(pair => !pair.Value.DisableAccountAutoCreation).Select(pair => pair.Key).ToArray();
    }
