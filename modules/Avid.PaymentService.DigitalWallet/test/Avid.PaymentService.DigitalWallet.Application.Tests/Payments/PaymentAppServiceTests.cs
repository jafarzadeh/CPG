...
namespace Avid.PaymentService.DigitalWallet.Payments;

public class PaymentAppServiceTests : DigitalWalletApplicationTestBase
{
    ...
    public async Task Should_Create_Payment()
    {
        ...
        await _distributedEventBus.PublishAsync(
            new CreatePaymentEto(null, DigitalWalletTestConsts.UserId,
                DigitalWalletPaymentServiceProvider.PaymentMethod, "CNY",
                new List<CreatePaymentItemEto>(new[]
                {
                    new CreatePaymentItemEto { ItemType = "Test", ItemKey = itemKey1, OriginalPaymentAmount = 50m },
                    new CreatePaymentItemEto { ItemType = "Test", ItemKey = itemKey2, OriginalPaymentAmount = 100m }
                })), false, false);
        ...
        payment.UserId.ShouldBe(DigitalWalletTestConsts.UserId);
        payment.PaymentMethod.ShouldBe(DigitalWalletPaymentServiceProvider.PaymentMethod);
    ...
    public async Task Should_Complete_Payment()
    {
        ...
        input.SetProperty(DigitalWalletConsts.PaymentAccountIdPropertyName, DigitalWalletTestConsts.AccountId);
        var account = await _accountRepository.GetAsync(DigitalWalletTestConsts.AccountId);
        ...
        account = await _accountRepository.GetAsync(DigitalWalletTestConsts.AccountId);
    ...
    public async Task Should_Refund_A_Part()
    {
            ...
            new RefundPaymentEto(null,
                new CreateRefundInput
                {
                    ...
                    RefundItems = new List<CreateRefundItemInput>
                    {
                        new()
                        {
                            PaymentItemId = item1.Id, RefundAmount = 10m, CustomerRemark = "Test3",
                            StaffRemark = "Test4"
                        }
                    }
                }), false, false);
        ...
        item1.PendingRefundAmount.ShouldBe(decimal.Zero);
        var refunds = await _refundAppService.GetListAsync(new GetRefundListInput { PaymentId = payment.Id });
        ...
        refund.RefundPaymentMethod.ShouldBe(DigitalWalletPaymentServiceProvider.PaymentMethod);
        ...
        paymentTransaction.AccountId.ShouldBe(DigitalWalletTestConsts.AccountId);
        paymentTransaction.ActionName.ShouldBe(DigitalWalletConsts.PaymentActionName);
        paymentTransaction.ChangedBalance.ShouldBe(-150m);
        paymentTransaction.OriginalBalance.ShouldBe(DigitalWalletTestConsts.AccountBaseBalance);
        paymentTransaction.PaymentId.ShouldBe(payment.Id);
        paymentTransaction.PaymentMethod.ShouldBe(DigitalWalletPaymentServiceProvider.PaymentMethod);
        paymentTransaction.TransactionType.ShouldBe(TransactionType.Credit);
        paymentTransaction.AccountUserId.ShouldBe(DigitalWalletTestConsts.UserId);
        ...
        refundTransaction.AccountId.ShouldBe(DigitalWalletTestConsts.AccountId);
        refundTransaction.ActionName.ShouldBe(DigitalWalletConsts.RefundActionName);
        refundTransaction.ChangedBalance.ShouldBe(+10m);
        refundTransaction.OriginalBalance.ShouldBe(DigitalWalletTestConsts.AccountBaseBalance +
                                                   paymentTransaction.ChangedBalance);
        refundTransaction.PaymentId.ShouldBe(payment.Id);
        refundTransaction.PaymentMethod.ShouldBe(DigitalWalletPaymentServiceProvider.PaymentMethod);
        refundTransaction.TransactionType.ShouldBe(TransactionType.Debit);
        refundTransaction.AccountUserId.ShouldBe(DigitalWalletTestConsts.UserId);
            ...
            new RefundPaymentEto(null,
                new CreateRefundInput
                {
                    ...
                    RefundItems = new List<CreateRefundItemInput>
                    {
                        new()
                        {
                            PaymentItemId = item1.Id, RefundAmount = item1.ActualPaymentAmount - item1.RefundAmount,
                            CustomerRemark = "Test3", StaffRemark = "Test4"
                        },
                        new()
                        {
                            PaymentItemId = item2.Id, RefundAmount = item2.ActualPaymentAmount,
                            CustomerRemark = "Test5", StaffRemark = "Test6"
                        }
                    }
                }), false, false);
        payment = await _paymentAppService.GetAsync(payment.Id);
