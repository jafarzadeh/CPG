...
public class TestController : PaymentServiceSampleController
{
    ...
    public TestController(IDistributedEventBus distributedEventBus, IPaymentAppService paymentAppService)
    {
            ...
            new CreatePaymentEto(CurrentTenant.Id, CurrentUser.GetId(), "Free", "CNY",
                new List<CreatePaymentItemEto>(new[]
                {
                    new CreatePaymentItemEto
                        { ItemType = "Test", ItemKey = Guid.NewGuid().ToString(), OriginalPaymentAmount = 0 }
                })), false, false);
    ...
    public async Task<PaymentDto> RefundAsync(Guid paymentId)
    {
        ...
        await _distributedEventBus.PublishAsync(
            new RefundPaymentEto(CurrentTenant.Id,
                new CreateRefundInput
                {
                    PaymentId = payment.Id, DisplayReason = "Test0", CustomerRemark = "Test1", StaffRemark = "Test2",
                    RefundItems = new List<CreateRefundItemInput>(payment.PaymentItems
                        ...
                        {
                            PaymentItemId = item.Id, RefundAmount = item.ActualPaymentAmount, CustomerRemark = "Test3",
                            StaffRemark = "Test4"
                        }))
                }), false, false);
